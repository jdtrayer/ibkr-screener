...
This exact bug has recurred twice: a new non-capacity admission hold gets
added to _try_admit's early-return checks, but _waiting_for_slot_count isn't
updated to exclude it, so a genuinely non-capacity-blocked symbol gets
reported as "waiting for a slot" -- see memory: waiting_count_conflation.
- 2026-08-31: _slot_cooldown wasn't excluded.
- 2026-09-03: _dead_hold and _ignored_until weren't excluded (reported live:
  22/35 slots used, 10 "waiting" -- all 10 were actually held, not
  capacity-blocked).
"""
...
def make_app(pending_symbols, dead_hold=(), ignored=(), cooldown=()):
